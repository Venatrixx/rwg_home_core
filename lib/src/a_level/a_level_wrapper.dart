...
class ALevelWrapper {
  ...
  ALevelWrapper.fromJson(dynamic json)
    : catalogName = json['catalogName'] ?? "APVO-MV (2019)",
      ...
      subjects = [
        for (final elem in json['subjects']) AbstractSubject.fromJson(elem),
      ],
      unknownSubjects = [
        for (final elem in json['unknownSubjects'])
          AbstractSubject.fromJson(elem),
      ],
  ...
  Map toJson() => {
    ...
    'unknownSubjects': [
      for (final subject in unknownSubjects) subject.toJson(),
    ],
  ...
  String catalogName;
  ValidationCatalog getCatalog() => availableValidationCatalogs.firstWhere(
  ...
  List<String> get advancedSubjectsNames {
    ...
    var advancedAbstractSubjects = subjects
        .where((s) => advancedSubjects.contains(s.abbr))
        .toList();
    return List.generate(
      advancedAbstractSubjects.length,
      (index) => advancedAbstractSubjects[index].name,
    );
  }
  ...
  List<String> get unknownSubjectsStrings {
    ...
    return List.generate(
      ...
      (index) =>
          "${unknownSubjects[index].name} (${unknownSubjects[index].abbr})",
    );
  }
  ...
  int get activeGradesCount {
    ...
    for (final elem in subjects) {
      count +=
          ...
          ((advancedSubjects.contains(elem.abbr) ? 2 : 1));
    }
    return count;
  }
  ...
  List<String> get allExamSubjects =>
      [...advancedSubjects, ?writtenExamSubject, ...oralExamSubjects]
  ...
  int? get finalResult2 {
    ...
    List<AbstractSubject> examSubjects = subjects
        .where((s) => allExamSubjects.contains(s.abbr))
        .toList();
    int sum = 0;
  ...
  List<String> get errors {
    ...
    for (final test in catalog.standardTests) {
      try {
  ...
  TaskStatus get otherExamSubjectsStatus {
    final catalog = getCatalog();
    if (catalog.examSubjectsTests.every(
      (cond) => cond.test(this) == TaskStatus.error,
    ))
      return TaskStatus.error;
    if (catalog.examSubjectsTests.any(
      (cond) => cond.test(this) == TaskStatus.error,
    )) {
      return TaskStatus.completeWithError;
    }
    if (catalog.examSubjectsTests.every(
      (cond) => cond.test(this) == TaskStatus.complete,
    )) {
      return TaskStatus.complete;
    }
  ...
  Future<void> initializeSubjects(List<Semester> semesters) async {
    final refSemester = semesters.firstWhere(
      (element) => element.hasData || element.hasFinalGrades,
    );

    unknownSubjects = [
      for (final sub in refSemester.subjects)
        AbstractSubject.fromSemesters(
          sub.name,
          sub.abbr.toLowerCase(),
          semesters,
        ),
    ];
    subjects = [];
  ...
  Future<void> verifySubjects() async {
    List<AbstractSubject> unknownSubjects = [
      ...this.subjects,
      ...this.unknownSubjects,
    ];
    List<AbstractSubject> subjects = [];
    unknownSubjects.removeWhere(
      (s) => hiddenSubjects.contains(s.abbr.toLowerCase()),
    );
  ...
  void updateSubjects(
    ...
    if (chooseOptimal == true) getCatalog().chooseOptimal?.call(this);
    if (sort == true) this.sort();

    for (final subject in subjects.where(
      (element) => allExamSubjects.contains(element.abbr),
    )) {
      for (final grade in subject.finalGrades) {
        grade.active = true;
      }
  ...
  void sort() {
    ...
    for (final abbr in allExamSubjects.reversed.toList()) {
      try {
        var subject = subjects.removeAt(
          subjects.indexOf(subjects.firstWhere((e) => e.abbr == abbr)),
        );
