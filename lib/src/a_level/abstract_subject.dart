...
class AbstractSubject {
  ...
  bool get hasData => finalGrades.any((element) => element.value != null) || examGrade != null;
  ...
  int get activeGradesCount => finalGrades.where((element) => element.active).length;

  List<AbstractFinalGrade> get finalGradesDesc => finalGrades..sort((a, b) => b.compareTo(a));

  List<AbstractFinalGrade> get finalGradesFilled => List.generate(4, (index) {
    var grade = finalGrades[index];
    return grade.copyWith(value: grade.value ?? avg?.round(), averaged: grade.value == null);
  });

  AbstractSubject({required this.name, required this.abbr, this.kind, required this.finalGrades, this.examGrade});

  factory AbstractSubject.fromSemesters(String name, String abbr, List<Semester> semesters) {
    List<AbstractFinalGrade> finalGrades = [];
      ...
      try {
        finalGrades.add(
          AbstractFinalGrade(
            index: index,
            value: sem.subjects
                .firstWhere((element) => element.abbr.toLowerCase() == abbr.toLowerCase())
                .finalSemesterGrade,
            active: false,
          ),
        );
  ...
  AbstractSubject.fromJson(dynamic json)
    : name = json['name'],
      abbr = json['abbr'],
      kind = json['kind'],
      finalGrades = [for (final elem in json['finalGrades']) AbstractFinalGrade.fromJson(elem)],
  ...
  Future<bool> normalize(List rawStructure) async {
    try {
      dynamic subjectStructure = rawStructure.firstWhere(
        ((element) => element['abbr'] == abbr || element['alt_abbr'].contains(abbr)),
      );
  ...
  void update(List<Semester> semesters, double Function(int) calculateExamWeight) {
      ...
      try {
        ...
        if (finalGrade == null) {
          final average = sem.subjects
              ...
              .round();
          res.add(AbstractFinalGrade(index: index, value: average, active: false));
        } else {
          res.add(AbstractFinalGrade(index: index, value: finalGrade, active: false));
        }
      } catch (_) {
